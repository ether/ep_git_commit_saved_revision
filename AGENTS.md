# Agent Guide — ep_git_commit_saved_revision

Hooks on Saved Revision calls, designed to git commit a pad text when a user clicks Saved Revision Star.
...
`ep_git_commit_saved_revision` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_git_commit_saved_revision`:
...
* Server: `eejsBlock_body`, `handleMessage`
* Client: `postAceInit`, `handleClientMessage_CUSTOM`
