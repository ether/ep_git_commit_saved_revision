# Contributing to ep_git_commit_saved_revision

Thanks for helping improve `ep_git_commit_saved_revision` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_git_commit_saved_revision` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_git_commit_saved_revision from your local clone
pnpm run plugins i --path ../ep_git_commit_saved_revision

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_git_commit_saved_revision/` are picked up after a server restart.
...
* Etherpad version, ep_git_commit_saved_revision version, browser + OS
* Steps to reproduce
